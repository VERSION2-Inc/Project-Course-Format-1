 ...
 * @param  bool    $iscopy         // TRUE:ファイルをコピーする, FALSE:リンクの書き換えのみ
    ...
    foreach ($mods as $mod) {
        ...
        if ($mod->section == $sectionid) {
            ...
            if (function_exists($mod->modname.'_get_links') && function_exists($mod->modname.'_rename_links')) {
                ...
                // 注意事項の追加
                if ($ismessage) {
                    notify(get_string('notrename','format_project', @$mod->modfullname));
                }
            }
        }
    }
 ...
 * @param  int     $sectionnumber  // 上書きするセクション番号（上書きする場合は重複していてもOK）
